...
<script>
import {
  detalheindicadorPrincipalid,
  EditarIndicadorPrincipal,
} from "../../api/indicadorPrincipal";
...
export default {
  ...
  created() {
    detalheindicadorPrincipalid(this.$route.query.id).then((response) => {
      this.form = response;
    });
  },
  methods: {
    onSubmit() {
      ...
      EditarIndicadorPrincipal(this.form).then((response) => {
